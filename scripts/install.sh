#!/bin/sh
# Copyright 2014-2018 Lacework Inc.
#
...
#    1. sudo sh -c "$(curl -sSL https://s3-us-west-2.amazonaws.com/www.lacework.net/download/2.9.17_2019-10-24_plume_4bd54a97d4ac8d5087390aa69adc0dbacd5f6f67/install.sh)" 
#    or
#    1. "curl -sSL https://s3-us-west-2.amazonaws.com/www.lacework.net/download/2.9.17_2019-10-24_plume_4bd54a97d4ac8d5087390aa69adc0dbacd5f6f67/install.sh > /tmp/install.sh"
#    2. sudo sh /tmp/install.sh
#    or
#    1. export LaceworkAccessToken="$1"
#    2. /usr/bin/docker run --name datacollector --net=host --pid=host --privileged --volume /:/laceworkfim:ro --volume /var/lib/lacework:/var/lib/lacework --volume /var/log:/var/log --volume /var/run:/var/run --volume /etc/passwd:/etc/passwd:ro --volume /etc/group:/etc/group:ro lacework/datacollector:latest
#
...
# Agent version
version=2.9.17
commit_hash=2.9.17_2019-10-24_plume_4bd54a97d4ac8d5087390aa69adc0dbacd5f6f67
deb_sha1=95b0d7af5666952d869561f3b8727d3830d53b53
rpm_sha1=5a4dd7323d629705443475423260ce23ea85d10f
dc_sha1=3cba18b3dd0029809b1de88809358bb578f4d8b6
...
install_signed_apt() {
	...
	( set -x; $sh_c "sleep 3; apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 18E76630; add-apt-repository 'deb [arch=amd64] ${download_url}/latest/DEB/$lsb_distro/$lsb_ver $lsb_rel main'; apt-get update; apt-get install lacework" )
}

install_signed_rpm() {
	( set -x; $sh_c "sleep 3; curl -sSL ${download_url}/latest/RPMS/x86_64/lacework-prod.repo > /tmp/lacework-prod.repo.$$; mv /tmp/lacework-prod.repo.$$ /etc/yum.repos.d/lacework-prod.repo; yum ${disable_epel} -y install /tmp/${pkg_fullname}")
}
...
#			fi
		...
		*fedora*|*centos*|*oraclelinux*|*redhatenterpriseserver*|*amzn*|*amazon*|*scientific*)
			...
			if [ "$lsb_dist" = "*fedora*" ] && [ "$dist_version" -ge "22" ]; then
				...
				( set -x; $sh_c "sleep 3; yum ${disable_epel} -y install /tmp/${pkg_fullname}")
			fi
		;;
		*coreos*|usedocker)
